...
class GroupConfigEntity {
  ...
  factory GroupConfigEntity.fromMap(Map<dynamic, dynamic> json) => GroupConfigEntity(
        splitzCategories: json["splitz_categories"] == null
            ...
            : List<SplitzCategory>.from(
                json["splitz_categories"].map((x) => SplitzCategory.fromMap(x)),
              ),
        splitzConfigs: json["splitz_configs"] == null
            ? {}
            : Map.from(json["splitz_configs"]).map(
                (k, v) => MapEntry<String, SplitzConfig>(k, SplitzConfig.fromMap(v)),
              ),
      );

  Map<String, dynamic> toMap() => {
        "splitz_categories": List.from(splitzCategories).map((x) => x.toMap()).toList(),
        "splitz_configs": Map.from(splitzConfigs).map((k, v) => MapEntry<String, dynamic>(k, v.toMap())),
      };
  ...
  GroupConfigEntity withPayer({
    ...
    if (users != null) {
      final payerUser = users.firstWhereOrNull(
        (e) => e.paidShare.isNotNullNorEmpty && double.parse(e.paidShare) != 0,
      );
...
class SplitzCategory {
  ...
  factory SplitzCategory.fromJson(String str) => SplitzCategory.fromMap(json.decode(str));
  ...
  Map<String, dynamic> toMap() => {
        ...
        "splitz_configs": splitzConfigs == null
            ? null
            : Map.from(splitzConfigs!).map((k, v) => MapEntry<String, dynamic>(k, v.toMap())),
      };
...
class SplitzConfig {
  ...
  factory SplitzConfig.fromJson(String str) => SplitzConfig.fromMap(json.decode(str));
